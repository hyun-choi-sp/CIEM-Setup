...
const PREFILL_CREDS = {
  iamUser: import.meta.env.VITE_AWS_IAM_USER || '',
  secretAccessKey: import.meta.env.VITE_AWS_SECRET_KEY || '',
}

export default function Step3ConfigureSourceCloudAws({ auth, initialConfig, onResult, onBack }) {
  const [form, setForm] = useState({
    ...
    clusterId: '',
    clusterName: '',
    connectorScriptName: 'aws-angularsc',
    iamUser: '',        // kid (Access Key ID)
    secretAccessKey: '', // secret
    roleName: 'SPServiceUserAccountAccess',
    region: 'us-east-1',
  })
  ...
  // Auto-fill owner with SailPoint Services
  useEffect(() => {
  ...
  const searchIdentities = useCallback(async (query) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIdentities([])
        setSearchError({ status: res.status, message: data?.detailCode || data?.message || `HTTP ${res.status}`, raw: text.slice(0, 400) })
      }
    } catch (err) {
      setIdentities([]); setSearchError({ status: null, message: err.message, raw: null })
    } finally {
      setSearchLoading(false); setShowDropdown(true)
    }
  }, [auth])

  const AWS_CLUSTER_KEYWORDS = ['aws cluster']

  const loadClusters = async () => {
    setClusterLoading(true); setClusters(null); setClusterError(null)
    try {
      const res = await fetch('/api/v3/managed-clusters?limit=50', {
      ...
      setForm((prev) => {
        ...
        const match = list.find((c) => AWS_CLUSTER_KEYWORDS.some((kw) => (c.name || '').toLowerCase().includes(kw)))
  ...
  const selectIdentity = (identity) => {
    ...
    setFieldErrors((fe) => ({ ...fe, owner: undefined }))
    setShowDropdown(false); setIdentities([])
  }
  ...
  const applyPrefill = () => {
    if (prefillActive) {
      setForm((f) => ({ ...f, iamUser: '', secretAccessKey: '' }))
      setPrefillActive(false)
    } else {
      setForm((f) => ({ ...f, ...PREFILL_CREDS }))
      setFieldErrors((fe) => ({ ...fe, iamUser: undefined, secretAccessKey: undefined }))
  ...
  const validate = () => {
    ...
    if (!form.iamUser.trim()) errs.iamUser = 'IAM User (Access Key ID) is required.'
    if (!form.secretAccessKey.trim()) errs.secretAccessKey = 'Secret Access Key is required.'
  ...
  const handleSubmit = async (e) => {
    ...
    /* ── Step B: Patch connection settings (auth) ── */
    setPhase('patching-auth')
    let patchAuthData = null

    try {
      const patchOps = [
        { op: 'add', path: '/connectorAttributes/authMethod', value: 'IAM User' },
        { op: 'add', path: '/connectorAttributes/kid', value: form.iamUser.trim() },
        { op: 'add', path: '/connectorAttributes/secret', value: form.secretAccessKey.trim() },
        { op: 'add', path: '/connectorAttributes/roleName', value: form.roleName.trim() },
        { op: 'add', path: '/connectorAttributes/externalId', value: null },
        { op: 'add', path: '/connectorAttributes/managementAccountId', value: null },
        { op: 'add', path: '/connectorAttributes/region', value: form.region.trim() || 'us-east-1' },
      ]
      ...
      const text = await res.text()
      try { patchAuthData = JSON.parse(text) } catch { patchAuthData = { _raw: text } }
      if (!res.ok) {
        setLoading(false); setPhase('idle')
        return onResult({ success: false, phase: 'patch_auth', sourceName: form.sourceName, sourceId, error: `HTTP ${res.status}: ${patchAuthData?.detailCode || patchAuthData?.message || text.slice(0, 300)}`, createResponse: createResponseData, rawResponse: patchAuthData }, savedConfig)
      }
    } catch (err) {
      setLoading(false); setPhase('idle')
      return onResult({ success: false, phase: 'patch_auth', sourceName: form.sourceName, sourceId, error: err.message, createResponse: createResponseData, rawResponse: null }, savedConfig)
    }

    /* ── Step C: Patch aggregation settings ── */
    setPhase('patching-agg')
    let patchAggData = null

    try {
      const patchOps = [
        { op: 'add', path: '/connectorAttributes/manageAllAccountsIAMData', value: true },
        { op: 'add', path: '/connectorAttributes/excludeAWSAccountIdList', value: null },
        { op: 'add', path: '/connectorAttributes/includeAWSAccountIdList', value: null },
        { op: 'add', path: '/connectorAttributes/pageSize', value: '10' },
        { op: 'add', path: '/connectorAttributes/assumeRoleSessionName', value: 'SailPointUser' },
        { op: 'add', path: '/connectorAttributes/assumeRoleDurationInSeconds', value: '3600' },
      ]
      const res = await fetch(`/api/beta/sources/${sourceId}`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${auth.token}`, 'Content-Type': 'application/json-patch+json', 'X-Tenant': auth.tenant },
      ...
      const text = await res.text()
      try { patchAggData = JSON.parse(text) } catch { patchAggData = { _raw: text } }
      if (!res.ok) {
        setLoading(false); setPhase('idle')
        return onResult({ success: false, phase: 'patch_aggregation', sourceName: form.sourceName, sourceId, error: `HTTP ${res.status}: ${patchAggData?.detailCode || patchAggData?.message || text.slice(0, 300)}`, createResponse: createResponseData, rawResponse: patchAggData }, savedConfig)
      }
    } catch (err) {
      setLoading(false); setPhase('idle')
      return onResult({ success: false, phase: 'patch_aggregation', sourceName: form.sourceName, sourceId, error: err.message, createResponse: createResponseData, rawResponse: null }, savedConfig)
    }
    ...
    try {
      ...
      if (!testPassed) {
        setLoading(false); setPhase('idle')
        return onResult({ success: false, phase: 'test_connection', sourceName: form.sourceName, sourceId, error: testErrorMsg, createResponse: createResponseData, rawResponse: testResponseData }, savedConfig)
      }
    } catch (err) {
      setLoading(false); setPhase('idle')
      return onResult({ success: false, phase: 'test_connection', sourceName: form.sourceName, sourceId, error: err.message, createResponse: createResponseData, rawResponse: null }, savedConfig)
    }
  ...
  const phaseLabel = { creating: 'Creating source…', 'patching-auth': 'Saving connection settings…', 'patching-agg': 'Saving aggregation settings…', testing: 'Testing connection…', aggregating: 'Triggering initial aggregation…' }
  const phaseDesc = { creating: 'Registering your AWS source with SailPoint ISC.', 'patching-auth': 'Applying IAM User credentials and role settings.', 'patching-agg': 'Applying account aggregation settings.', testing: 'Verifying connectivity between SailPoint and AWS.', aggregating: 'Starting account correlation and initial aggregation.' }
  const phaseStep = { creating: 'Step 1/5', 'patching-auth': 'Step 2/5', 'patching-agg': 'Step 3/5', testing: 'Step 4/5', aggregating: 'Step 5/5' }
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
      ...
      <div className="px-8 py-6 border-b border-gray-100 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Configure your AWS source with IAM User authentication.</p>
        </div>
      </div>
        ...
        <div className="px-8 py-7 space-y-8">
            ...
            <div className="space-y-5">
              ...
              <FieldWrapper label="Source Owner" required error={fieldErrors.owner} hint="Type 2+ characters to search identities in your tenant.">
                ...
                {form.ownerId && (
                  <p className="mt-1.5 text-xs text-green-600 font-medium flex items-center gap-1">
                    <CheckCircleIcon className="w-3.5 h-3.5" />{form.ownerName}<span className="text-gray-400 font-normal">({form.ownerId})</span>
                  </p>
                )}
              </FieldWrapper>
              ...
              <FieldWrapper label="Virtual Appliance Cluster" required error={fieldErrors.clusterId} hint="The VA cluster that will handle this source's connectivity.">
                ...
                {form.clusterId && clusters && (
                  <p className="mt-1.5 text-xs text-green-600 flex items-center gap-1"><CheckCircleIcon className="w-3.5 h-3.5" />{form.clusterName || form.clusterId}</p>
                )}
              </FieldWrapper>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-5">
              ...
              <FieldWrapper label="Authentication Method" hint="Fixed to IAM User for this source type.">
                <input type="text" readOnly value="IAM User" className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 text-gray-500 cursor-not-allowed" />
              </FieldWrapper>

              <FieldWrapper label="IAM User (Access Key ID)" required error={fieldErrors.iamUser} hint="The AWS Access Key ID for the IAM User. Refer to Keeper: AWS Source for CIEM Configuration.">
                <input type="text" value={form.iamUser} onChange={setField('iamUser')} placeholder="AKIA…" className={inputCls('iamUser') + ' font-mono'} disabled={loading} spellCheck={false} autoComplete="off" />
              </FieldWrapper>

              <FieldWrapper label="Secret Access Key" required error={fieldErrors.secretAccessKey} hint="Paste carefully — avoid leading/trailing whitespace. Refer to Keeper: AWS Source for CIEM Configuration.">
                <div className="relative">
                  <input type={showSecrets.secretAccessKey ? 'text' : 'password'} value={form.secretAccessKey} onChange={setField('secretAccessKey')} placeholder="Secret Access Key" className={inputCls('secretAccessKey') + ' font-mono pr-10'} disabled={loading} autoComplete="new-password" />
                  <button type="button" tabIndex={-1} onClick={() => setShowSecrets((s) => ({ ...s, secretAccessKey: !s.secretAccessKey }))} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                    {showSecrets.secretAccessKey ? <EyeOffIcon className="w-4 h-4" /> : <EyeIcon className="w-4 h-4" />}
                  </button>
                </div>
              </FieldWrapper>

              <FieldWrapper label="Role Name" required error={fieldErrors.roleName} hint="The IAM role name that SailPoint will assume.">
                <input type="text" value={form.roleName} onChange={setField('roleName')} placeholder="SPServiceUserAccountAccess" className={inputCls('roleName') + ' font-mono'} disabled={loading} spellCheck={false} />
              </FieldWrapper>

              <FieldWrapper label="Region" hint="AWS region. Defaults to us-east-1.">
                <input type="text" value={form.region} onChange={setField('region')} placeholder="us-east-1" className={inputCls('region') + ' font-mono'} disabled={loading} spellCheck={false} />
              </FieldWrapper>
            </div>
          </section>
          ...
          {/* ── Aggregation Settings (read-only info) ── */}
          <section>
            <SectionHeading>Aggregation Settings</SectionHeading>
            <div className="rounded-xl bg-gray-50 border border-gray-200 px-5 py-4 space-y-2">
              {[
                ['Manage All Accounts', 'On'],
                ['Page Size', 'Default (10)'],
                ['Role Session Name', 'SailPointUser'],
                ['Role Session Duration', '3600 seconds'],
              ].map(([label, val]) => (
                <div key={label} className="flex items-center justify-between text-sm">
                  <span className="text-gray-500">{label}</span>
                  <span className="font-medium text-gray-700 font-mono text-xs">{val}</span>
                </div>
              ))}
              <p className="text-xs text-gray-400 pt-1">These defaults match the standard configuration. They will be applied automatically.</p>
            </div>
          </section>
          ...
          <section>
            ...
            <FieldWrapper label="Connector Script Name" required error={fieldErrors.connectorScriptName}>
              <input type="text" value={form.connectorScriptName} onChange={setField('connectorScriptName')} placeholder="aws-angularsc" className={inputCls('connectorScriptName') + ' font-mono'} disabled={loading} spellCheck={false} />
            </FieldWrapper>
          </section>
        </div>
        ...
        {loading && (
          <div className="mx-8 mb-4">
            <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-blue-800">{phaseLabel[phase]}</p>
                <p className="text-xs text-blue-600 mt-0.5">{phaseDesc[phase]}</p>
                <p className="text-xs text-blue-500 mt-1 font-mono">{phaseStep[phase]}</p>
              </div>
            </div>
          </div>
        )}
...
function FieldWrapper({ label, required, badge, hint, error, children }) {
  return (
    <div>
      ...
      {children}
      {error ? (
        <p className="mt-1.5 text-xs text-red-600 flex items-center gap-1"><ExclamationIcon className="w-3.5 h-3.5 flex-shrink-0" />{error}</p>
      ) : hint ? (
...
function Spinner({ className }) {
  return <svg className={`animate-spin w-4 h-4 ${className || ''}`} fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
}
