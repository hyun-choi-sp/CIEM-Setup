...
const PREFILL_CREDS = {
  clientId: import.meta.env.VITE_GSUITE_CLIENT_ID || '',
  clientSecret: import.meta.env.VITE_GSUITE_CLIENT_SECRET || '',
  refreshToken: import.meta.env.VITE_GSUITE_REFRESH_TOKEN || '',
}

export default function Step3ConfigureSourceCloudGsuite({ auth, initialConfig, onResult, onBack }) {
  const [form, setForm] = useState({
    ...
    clusterId: '',
    clusterName: '',
    connectorScriptName: 'g-suite-angularsc',
    clientId: '',
    clientSecret: '',
    refreshToken: '',
    organizationId: '1063145834985',
    enableGcpSupport: true,
  })
  ...
  const GCP_CLUSTER_KEYWORDS = ['aws cluster']
  ...
  const loadClusters = async () => {
    ...
    try {
      ...
      setForm((prev) => {
        ...
        const match = list.find((c) => GCP_CLUSTER_KEYWORDS.some((kw) => (c.name || '').toLowerCase().includes(kw)))
  ...
  const applyPrefill = () => {
    if (prefillActive) {
      setForm((f) => ({ ...f, clientId: '', clientSecret: '', refreshToken: '' }))
      setPrefillActive(false)
    } else {
      setForm((f) => ({ ...f, ...PREFILL_CREDS }))
      setFieldErrors((fe) => ({ ...fe, clientId: undefined, clientSecret: undefined, refreshToken: undefined }))
  ...
  const validate = () => {
    ...
    if (!form.refreshToken.trim()) errs.refreshToken = 'Refresh Token is required.'
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const patchOps = [
        ...
        { op: 'add', path: '/connectorAttributes/refreshToken', value: form.refreshToken.trim() },
        { op: 'add', path: '/connectorAttributes/jwt_issuer', value: null },
        { op: 'add', path: '/connectorAttributes/jwt_subject', value: null },
        { op: 'add', path: '/connectorAttributes/jwt_scope', value: null },
        { op: 'add', path: '/connectorAttributes/private_key', value: null },
        { op: 'add', path: '/connectorAttributes/private_key_password', value: null },
        { op: 'add', path: '/connectorAttributes/enableGcpSupport', value: form.enableGcpSupport },
        { op: 'add', path: '/connectorAttributes/organizationId', value: form.organizationId.trim() },
      ]
    ...
    /* ── Step D: Mark connected + trigger account aggregation ── */
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
      ...
      <div className="px-8 py-6 border-b border-gray-100 flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-semibold text-gray-900">G Suite Configuration</h2>
          <p className="text-sm text-gray-500 mt-0.5">Configure your G Suite source with OAuth2 and GCP Cloud Resources support.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="px-8 py-7 space-y-8">
          ...
          <section>
            ...
            <div className="space-y-5">
              ...
              <FieldWrapper label="Client ID" required error={fieldErrors.clientId} hint="Refer to Keeper: GCP Source for CIEM Configuration.">
                <input type="text" value={form.clientId} onChange={setField('clientId')} placeholder="Client ID" className={inputCls('clientId') + ' font-mono'} disabled={loading} spellCheck={false} autoComplete="off" />
              </FieldWrapper>

              <FieldWrapper label="Client Secret" required error={fieldErrors.clientSecret} hint="Refer to Keeper: GCP Source for CIEM Configuration.">
              ...
              <FieldWrapper label="Refresh Token" required error={fieldErrors.refreshToken} hint="Refer to Keeper: GCP Source for CIEM Configuration.">
                <div className="relative">
                  <input type={showSecrets.refreshToken ? 'text' : 'password'} value={form.refreshToken} onChange={setField('refreshToken')} placeholder="Refresh Token" className={inputCls('refreshToken') + ' font-mono pr-10'} disabled={loading} autoComplete="new-password" />
                  <button type="button" tabIndex={-1} onClick={() => setShowSecrets((s) => ({ ...s, refreshToken: !s.refreshToken }))} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                    {showSecrets.refreshToken ? <EyeOffIcon className="w-4 h-4" /> : <EyeIcon className="w-4 h-4" />}
                  </button>
                </div>
              </FieldWrapper>

              <FieldWrapper label="Organization ID" hint="Your GCP Organization ID (numeric).">
                <input type="text" value={form.organizationId} onChange={setField('organizationId')} placeholder="1063145834985" className={inputCls('organizationId') + ' font-mono'} disabled={loading} spellCheck={false} />
              </FieldWrapper>

              <FieldWrapper label="Manage Cloud Resources" hint="Enable GCP resource management (recommended).">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={form.enableGcpSupport}
                    onChange={(e) => setForm((f) => ({ ...f, enableGcpSupport: e.target.checked }))}
                    disabled={loading}
                    className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-sm text-gray-600">Manage Cloud Resources</span>
                </label>
              </FieldWrapper>
            </div>
          </section>
          ...
          <section>
            ...
            <FieldWrapper label="Connector Script Name" required error={fieldErrors.connectorScriptName}>
              <input type="text" value={form.connectorScriptName} onChange={setField('connectorScriptName')} placeholder="g-suite-angularsc" className={inputCls('connectorScriptName') + ' font-mono'} disabled={loading} spellCheck={false} />
            </FieldWrapper>
          </section>
        </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
              ...
              <div>
                ...
                <p className="text-xs text-blue-600 mt-0.5">
                  {phase === 'creating' && 'Registering your G Suite source with SailPoint ISC.'}
                  {phase === 'patching' && 'Applying OAuth2 credentials and GCP settings.'}
                  {phase === 'testing' && 'Verifying connectivity between SailPoint and Google.'}
        ...
        <div className="px-8 pb-8 flex gap-3">
          ...
          <button type="submit" disabled={loading} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-semibold py-2.5 px-4 rounded-xl transition-colors flex items-center justify-center gap-2">
            {loading ? (
              <><Spinner />{phase === 'creating' ? 'Creating Source…' : phase === 'patching' ? 'Saving Settings…' : phase === 'aggregating' ? 'Starting Aggregation…' : 'Testing Connection…'}</>
            ) : (
